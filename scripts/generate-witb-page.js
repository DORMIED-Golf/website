...
function writeWitbLeadersData({ players, currentItems }) {
  ...
  const rankedBagIds  = new Set(rankedPlayers.map(p => p.current_bag_id).filter(Boolean));
  const rankedCurrentItems = currentItems.filter(i => rankedBagIds.has(i.bag_id));
  // Top 10 ranked players (includes top 5 needed for leaders + 10 for most-viewed fallback)
  ...
  const payload = {
    ...
    topPlayers:   topPlayers.slice(0, 5),  // top 5 for WITB LEADERS column
    allPlayers:   topPlayers,              // top 10 for Most Viewed WITBs fallback
    topDrivers,
    topPutters,
  };
  ...
  console.log(`\nWrote: js/witb-leaders.js (${topPlayers.length} players, ${topDrivers.length} drivers, ${topPutters.length} putters)`);
}
